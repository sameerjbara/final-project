...
────────────────────
Pulls news articles from Polygon.io Reference News API.
- Clean financial news only — Zacks, Benzinga, Motley Fool, etc.
- Uses cursor-based pagination (next_url) for reliable paging
...
- Free tier: unlimited calls, 2 years history
...
USAGE:
    ...
    python src/collection/polygon_collector.py --tickers AAPL NVDA

    # Expand date range (skips already-pulled months)
    python src/collection/polygon_collector.py --start 2024-05

    # Merge all monthly files into one CSV
    python src/collection/polygon_collector.py --merge

    # Full run + merge in one command
    python src/collection/polygon_collector.py --merge --start 2024-05

    # Check coverage without pulling
    python src/collection/polygon_collector.py --coverage

    # Pull specific dates for one ticker
    python src/collection/polygon_collector.py --tickers AAPL --dates 2025-02-02 2025-02-09
"""
...
DELAY     = 1.0       # seconds between requests (free tier is generous)
PAGE_SIZE = 1000      # max articles per request (Polygon supports up to 1000)
MAX_PAGES = 20        # safety cap to avoid infinite loops
...
# Output paths
SAVE_DIR    = "data/raw/polygon_monthly"
OUTPUT_FILE = "data/raw/polygon_raw.csv"
...
# ── API Key rotation ──────────────────────────────────────────────────────────
# Add POLYGON_API_KEY_1, POLYGON_API_KEY_2 etc. to your .env file
def load_api_keys() -> list:
    ...
    if not keys:
        raise ValueError(
            "No Polygon API keys found. "
            "Add POLYGON_API_KEY_1 to your .env file. "
...
def rotate_key() -> bool:
    """
    Rotate to next key cyclically.
    Waits 60 seconds when all keys have been tried once,
    then starts from key 1 again.
    """
    ...
    if current_key_idx < len(API_KEYS) - 1:
        ...
        # All keys exhausted — wait and cycle back to key 1
        print(f"    ⚠ All {len(API_KEYS)} keys hit limit — waiting 60s then retrying from key 1")
        time.sleep(60)
        current_key_idx = 0
        print(f"    ↻ Reset to API key 1/{len(API_KEYS)}")
        return True
...
def fetch_page(url: str, params: dict = None) -> dict | None:
    """
    Fetches one page from Polygon API.
    Handles both initial requests (with params) and
    cursor-based next_url requests.
    Returns response dict or None on error.
    """
    while True:
        try:
            if params:
                # Initial request — add apiKey to params
                params["apiKey"] = get_current_key()
                response = requests.get(url, params=params, timeout=30)
            else:
                # Cursor-based next_url — apiKey already in URL
                # Replace the key in the URL with current key
                if "apiKey=" in url:
                    url = url.split("apiKey=")[0] + f"apiKey={get_current_key()}"
        ...
        if response.status_code == 403:
            print(f"\n    ✖ Auth error on key {current_key_idx + 1} — rotating")
            if rotate_key():
                continue
            else:
                return None
...
def pull_month(ticker: str, year: int, month: int) -> pd.DataFrame | None:
    """
    Pulls one month of Polygon news for one ticker.
    Uses cursor-based pagination via next_url.
    ...
    while page_count < MAX_PAGES:
        data = fetch_page(url, params if page_count == 0 else None)
...
def pull_specific_dates(ticker: str, dates: list) -> pd.DataFrame:
    """
    Pulls Polygon articles for a specific ticker on specific dates.
    ...
    output_path = os.path.join("data/raw", f"{ticker}_polygon_specific_dates.csv")
...
def run_collection(
    ...
    """
    Pulls Polygon news month by month for all tickers.
    Returns True if completed fully, False if stopped early.
    """
    ensure_dirs()
    months = get_months_in_range(start, end)

    print(f"── POLYGON COLLECTION ───────────────────────────────────")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Pull Polygon.io news data")
    ...
    if args.dates:
        if len(args.tickers) != 1:
            ...
            print("  Example: python src/collection/polygon_collector.py "
        ...
        if completed and args.merge:
            ...
            print(f"  python src/collection/polygon_collector.py --merge")